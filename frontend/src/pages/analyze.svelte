		...
		<div class="max-w-3xl mx-auto flex flex-col gap-8">
			...
			<div>
				...
				<p class="mt-1 text-sm text-zinc-500">
					Upload a call recording or transcript to get an AI-powered
					quality review.
				</p>
			</div>
			...
			<section class="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
				...
				<ol class="flex flex-col gap-2">
					{#each ["Upload an audio file (.mp3, .wav, .m4a) or a plain-text transcript (.txt).", "The AI will transcribe and review the call against quality standards.", "Review the summary, highlights, and scorecard below."] as step, i}
						<li
							class="flex items-start gap-3 text-sm text-zinc-400"
						>
							<span
								class="flex items-center justify-center w-5 h-5 rounded-full bg-zinc-800 text-zinc-300 text-xs font-semibold shrink-0 mt-0.5"
								>{i + 1}</span
							>
							{step}
						</li>
			...
			<section>
				...
				<div
					...
					ondrop={(e) => {
						e.preventDefault();
						dragging = false;
						fileName = e.dataTransfer?.files[0]?.name ?? null;
					}}
				>
					<div
						class="flex items-center justify-center w-12 h-12 rounded-full bg-zinc-800"
					>
						<svg
					...
					{#if fileName}
						<div>
							...
							<p class="text-xs text-zinc-500 mt-0.5">
								File ready — click Analyze to continue
							</p>
						</div>
					{:else}
						<div>
							...
							<p class="text-xs text-zinc-500 mt-0.5">
								or click to browse — .mp3, .wav, .m4a, .txt
							</p>
						</div>
					{/if}
					<input
			...
			<section class="flex flex-col gap-5">
				...
				<div class="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
					...
					<p class="text-sm text-zinc-400 leading-relaxed">
						The agent handled an inbound billing inquiry from a
						returning customer. The core issue — an unexpected
						charge on the monthly statement — was identified and
						resolved within the call. The agent demonstrated solid
						product knowledge but missed opportunities to confirm
				...
				<div class="grid grid-cols-2 gap-4">
					<!-- Pluses -->
					<div
						class="rounded-xl border border-emerald-900/60 bg-emerald-950/30 p-5"
					>
					...
					<!-- Minuses -->
					<div
						class="rounded-xl border border-red-900/60 bg-red-950/20 p-5"
					>
						<div class="flex items-center gap-2 mb-3">
							<svg
								xmlns="http://www.w3.org/2000/svg"
						...
						<ul class="flex flex-col gap-2">
							{#each minuses as minus}
								...
								>
									<span
										class="mt-1.5 w-1.5 h-1.5 rounded-full bg-red-500 shrink-0"
									></span>
									{minus}
								</li>
							{/each}
				...
				<div class="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
					<div class="flex items-center justify-between mb-4">
						...
						<div class="flex items-center gap-2">
							...
							<span
								class="text-lg font-bold {overallScore >= 8
									? 'text-emerald-400'
									: overallScore >= 6
										? 'text-amber-400'
										: 'text-red-400'}"
								>{overallScore}<span
									class="text-xs font-normal text-zinc-600"
									>/10</span
								></span
							>
						</div>
					...
					<ul class="flex flex-col gap-3">
						{#each scorecard as row}
							...
							<li class="flex items-center gap-3">
								<span
									class="w-40 shrink-0 text-sm text-zinc-400"
									>{row.label}</span
								>
								<div
									class="flex-1 h-1.5 rounded-full bg-zinc-800 overflow-hidden"
								>
									<div
										class="h-full rounded-full transition-all duration-500 {row.score >=
										8
											? 'bg-emerald-500'
											: row.score >= 6
												? 'bg-amber-400'
								...
								<span
									class="w-8 text-right text-sm font-medium {row.score >=
									8
										? 'text-emerald-400'
										: row.score >= 6
											? 'text-amber-400'
											: 'text-red-400'}">{row.score}</span
								>
							</li>
						{/each}
					</ul>
